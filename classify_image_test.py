...
def run_inference_on_image_file(file_dir):
  ...
  with tf.Session() as sess:
      for name in os.listdir(file_dir):
          #start = time.time()
          image_data = tf.gfile.FastGFile(file_dir+name, 'rb').read()
          image_data = tf.image.decode_jpeg(image_data)
          image_data = preprocess_for_eval(image_data, 299, 299)
          image_data = tf.expand_dims(image_data, 0)
          if 1:
            image_data = sess.run(image_data)
            softmax_tensor = sess.graph.get_tensor_by_name('MobilenetV2/Predictions/Softmax:0')
...
def run_inference_on_image(image):
  ...
  with tf.Session() as sess:
    softmax_tensor = sess.graph.get_tensor_by_name('MobilenetV2/Logits/Squeeze:0')
    predictions = sess.run(softmax_tensor,
                           {'input:0': image_data})
    predictions = np.squeeze(predictions)
...
def main(_):
  ...
  #run_inference_on_image_file(file_dir)
